 ...
 ** Copyright (C) 2024 Ivan Alonso (aka Kaian)
 **
 ...
 *
 * @brief Module for manage conference meets through AMI using ConfBridge
 *
...
conference_guest_state(Filter *filter, AmiMessage *msg)
{
    ...
    if (!strcasecmp(event, "VarSet")) {
        ...
        if (!strcasecmp(varname, "ACTIONID")) {
            ...
            if (!guest) {
                isaac_log(LOG_ERROR, "Unable to find conference guest for actionid %s.", message_get_header(msg, "Value"));
                return 0;
            }
            ...
            filter_new_condition(guest_filter, MATCH_REGEX, "Event", "Hangup$|ConfbridgeJoin|ConfbridgeLeave");
            filter_new_condition(guest_filter, MATCH_EXACT_CASE, "UniqueID", guest->uid);
    ...
    } else if (!strcasecmp(event, "ConfbridgeJoin")) {
        session_write(filter->sess,
                      "CONFERENCESTATUS %s %s JOINED\n",
                      info->actionid,
                      guest->extension
        );
    } else if (!strcasecmp(event, "ConfbridgeLeave")) {
        session_write(filter->sess,
...
conference_host_state(Filter *filter, AmiMessage *msg)
{
    ...
    if (!strcasecmp(event, "VarSet")) {
        ...
        if (!strcasecmp(varname, "ACTIONID")) {
            ...
            filter_new_condition(host_filter, MATCH_REGEX, "Event", "Hangup$|ConfbridgeJoin");
            filter_new_condition(host_filter, MATCH_EXACT_CASE, "UniqueID", info->host_uid);
            ...
            filter_new_condition(conference_end_filter, MATCH_EXACT, "Event", "ConfbridgeEnd");
            filter_new_condition(conference_end_filter, MATCH_EXACT, "Conference", info->conference_id);
    ...
    } else if (!strcasecmp(event, "ConfbridgeEnd")) {
        session_write(filter->sess,
